...
import GoPuff from '../Images/Gopuff.jpg'
...
export const ProductCard = ({ name, description, storeLocations }: ProductCardProps) => {
  ...
  const GoPuffStoreData = [
    
    { 
      name: "GoPuff", 
      location: "Fort Lauderdale, FL", 
      distance: "1110 NE 8th Ave", 
      price: "$3.",
      cents: "99",
      logo: GoPuff
    },
  ...
  const totalPrice = OnlineStoreData.reduce((sum, store) => {
  ...
  const handleStoreClick = (storeType: 'online' | 'gopuff', index: number) => {
    const stateKey = `${storeType}-${index}`;
                ...
                {activeTab === 'purchase' ? (
                  ...
                  <AccordionContent>
                    <div className="space-y-4 mt-4 mb-6">
                      {OnlineStoreData.map((store, index) => (
                        <div key={index} className="flex items-center justify-between p-3 rounded-xl border border-gray-200 hover:border-emerald-200 transition-colors duration-200">
                          ...
                          <Button
                            size="sm"
                            onClick={() => handleStoreClick('online', index)}
                            disabled={processingStates[`online-${index}`]}
                            className={`bg-gradient-to-br from-emerald-400 to-emerald-500 hover:from-emerald-500 hover:to-emerald-600 text-white rounded-full transition-all duration-200 shadow shadow-emerald-500/20 hover:shadow-emerald-500/30 active:scale-95 ${processingStates[`online-${index}`] ? 'opacity-70 cursor-not-allowed' : ''}`}
                          >
                            <div className='relative'>
                              {processingStates[`online-${index}`] ? '' : `${store.price}`}
                            </div>
                              <div className='relative -ml-2 mt-0.2'>
                                <span className="text-xs">{processingStates[`online-${index}`] ? 'Confirmed..' : `${store.cents} / pack`}</span>
                              </div>
                          </Button>
                        </div>
                      ))}
                       {GoPuffStoreData.map((store, index) => (
                        <div key={index} className="flex items-center justify-between p-3 rounded-xl border border-gray-200 hover:border-emerald-200 transition-colors duration-200">
                          <div className="flex items-center gap-3">
                            <div className="min-w-[2.75rem] w-11 h-11 rounded-full bg-gray-100 flex items-center justify-center p-1.5 shrink-0">
                              <Image
                          ...
                          <Button
                            size="sm"
                            onClick={() => handleStoreClick('gopuff', index)}
                            disabled={processingStates[`gopuff-${index}`]}
                            className={`bg-gradient-to-br from-emerald-400 to-emerald-500 hover:from-emerald-500 hover:to-emerald-600 text-white rounded-full transition-all duration-200 shadow shadow-emerald-500/20 hover:shadow-emerald-500/30 active:scale-95 ${processingStates[`gopuff-${index}`] ? 'opacity-70 cursor-not-allowed' : ''}`}
                          >
                            <div className='relative'>
                              {processingStates[`gopuff-${index}`] ? '' : `${store.price}`}
                            </div>
                              <div className='relative -ml-2 mt-0.2'>
                                <span className="text-xs">{processingStates[`gopuff-${index}`] ? 'Confirmed..' : `${store.cents} / can`}</span>
                              </div>
                          </Button>
                        </div>
                      ))}
