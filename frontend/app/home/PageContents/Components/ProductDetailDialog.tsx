...
export function ProductDetailDialog({ product, open, onOpenChange }: ProductDetailProps) {
  ...
  const getProductRoute = () => {
    if (product.brand === "JOYRIDE") return "/joyride";
    if (product.brand === "NUTCASE") return "/Nutcase";
    if (product.brand === "Ketone-IQ") return "/ketone";
    if (product.brand === "ItsCalledW") return "/itscalledw";
    return "/";
  };
