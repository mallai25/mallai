...
function ProductContent() {
  ...
  const [selectedProduct, setSelectedProduct] = useState({
    ...
    website: "",
    similarProducts: [],
    influencer: null // Add this to store influencer data
  });
  ...
  const handleBackgroundImageUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (imageUrl && selectedProduct.id) {
        ...
        const docRef = doc(FIREBASE_DB, 'listingsMade', selectedProduct.id.toString());
  ...
  useEffect(() => {
    const fetchProductData = async () => {
      ...
      try {
        ...
        if (decodedData.id) {
          const docRef = doc(FIREBASE_DB, 'listingsMade', decodedData.id);
          ...
          if (docSnap.exists()) {
            ...
            // If there's an influencer reference, fetch influencer data
            let influencerData = null;
            if (productData.influencer) {
              const influencerRef = doc(FIREBASE_DB, 'influencers', productData.influencer.id);
              const influencerSnap = await getDoc(influencerRef);
              if (influencerSnap.exists()) {
                influencerData = influencerSnap.data();
              }
            }
            ...
            setSelectedProduct({
              ...
              imageUrl: productData.imageUrl || productData.imageSrc || "",
              description: productData.description || "",
              gtin: productData.gtin || "",
              weight: productData.weight || "",
              brandImageUrl: productData.brandImageUrl || "",
              backgroundImageUrl: influencerData?.image || productData.backgroundImageUrl || "", // Use influencer image as background
            ...
            // Fetch polls and campaigns if userId exists
            ...
            // Set background image to influencer image if available
            setBackgroundImage(influencerData?.image || productData.backgroundImageUrl || null);
          }
        }
      } catch (error) {
