...
import { CheckoutButton } from "./Stripe/CheckoutButton" // Import the CheckoutButton component
...
export default function JoinPage() {
  ...
  // This function will be used to handle successful checkout
  ...
  const addSimilarItemToCart = (parentProduct, similarItem) => {
    ...
    // Get priceUnit information from the product or similar item
    const priceUnit =
      similarItem.priceUnit ||
      parentProduct.priceUnit ||
      (parentProduct.brand === "ItsCalledW"
        ? "stick"
        : parentProduct.brand === "Ketone-IQ"
          ? "serving"
          : parentProduct.category === "Beverage"
            ? "pack"
            : "bag")
  ...
  return (
    <div className="min-h-screen bg-white relative">
                  ...
                  {cartItems.length === 0 ? (
                    ...
                    <div className="space-y-4 flex-1 flex flex-col">
                                  ...
                                  <div className="flex justify-between items-center mt-2">
                                    <div className="flex items-center">
                                      <div className="flex items-center border rounded-lg h-8">
                                        ...
                                        <div className="w-auto px-2 text-center text-sm">
                                      ...
                                      {item.priceUnit && (
                                        <span className="text-xs text-gray-500 ml-2">/{item.priceUnit}</span>
                                      )}
                                    </div>
                      ...
                      <div className="border-t pt-4 space-y-4 mt-auto">
                        ...
                        {/* Replace the Button with CheckoutButton component */}
                        <CheckoutButton
                          cartItems={cartItems}
                          className="w-full py-6 rounded-2xl bg-gradient-to-r from-emerald-500 to-teal-600 text-white font-medium text-base shadow-md hover:opacity-95 active:opacity-90 transition-opacity duration-200"
                        />
                      </div>
                    </div>
                  )}
                                ...
                                <div className="flex justify-between">
                                  <div>
                                    ...
                                    {item.isSimilarItem && item.parentProductName && (
                                      <p className="text-xs text-blue-600">From: {item.parentProductName}</p>
                                    )}
                                    {item.description && (
      ...
      {/* Add these components right before the closing </div> of the main component */}

      <SuccessPopup open={showSuccessPopup} onClose={() => setShowSuccessPopup(false)} orderDetails={orderDetails} />
    </div>
  )
}
