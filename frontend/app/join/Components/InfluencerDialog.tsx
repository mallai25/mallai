...
export function InfluencerDialog({ influencer, product, open, onOpenChange }: InfluencerDialogProps) {
  if (!influencer) return null;

  const getCampaignRoute = () => {
    // Handle default brands
    switch (influencer.brand) {
      case "JOYRIDE":
        return "/joyride";
      case "NUTCASE":
        return "/Nutcase";
      ...
      case "ItsCalledW":
        return "/itscalledw";
      default:
        // For other brands, use the product id from the passed product data
        return "#";
        // ?.id ? `/prelim?data=${encodeURIComponent(JSON.stringify({ id: product.id }))}` : "/";
    }
  };

  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[715px] overflow-y-auto max-h-[90vh] p-3 sm:p-6 rounded-2xl sm:rounded-2xl">
                    ...
                    <Card className={`overflow-hidden group transition-all duration-200 hover:shadow-sm ${account.lightBg} hover:scale-105`}>
                      <CardContent className="p-2 sm:p-3 flex items-center justify-center">
                        <div className="flex items-center space-x-2">
                          ...
                          <span className="text-xs sm:text-sm font-medium">{account.name.replace(" Profile", "").replace(" Channel", "").replace("Instagram", "IG").replace("Youtube", "YouTube")}</span>
                        </div>
                      </CardContent>
                    </Card>
                  </a>
        ...
        {(influencer.brand === "JOYRIDE" || 
    influencer.brand === "NUTCASE" || 
    influencer.brand === "KETONE-IQ" || 
    influencer.brand === "ItsCalledW") && (
    <Button 
